...
from call_langchain import call_langchain_agent
...
if uploaded_file is not None:
    ...
    if prompt := st.chat_input("Enter your prompt:"):
        ...
        with st.chat_message("assistant"):
            response, st.session_state.num_clarifying_questions_asked = call_langchain_agent(df, langchain_llm, st.session_state.messages, st.session_state.num_clarifying_questions_asked, st.session_state.max_clarifying_questions)
            ...
            if all(isinstance(fig, Figure) for fig in response):
                for fig in response:
                    st.plotly_chart(fig)  
            elif all(isinstance(dataframe, pd.DataFrame) for dataframe in response):
