...
def bland_altman_plot(df, endpoint1, endpoint2, bySeverityCategory=False) -> str:
    ...
    if bySeverityCategory:
        ...
        for category in severity_categories:
            ...
            fig = plot_bland_altman(category_data, category)
        ...
        fig = plot_bland_altman(df_merged)
        plot_results.append(fig)
...
def two_endpoints_visualization_report(df, endpoint1, endpoint2, gold_standard_endpoint, bySeverityCategory=False):
    ...
    def add_to_plot_results(input_data, plot_results):
        if isinstance(input_data, list):
            plot_results.extend(input_data)
        else:
            plot_results.append(input_data)
            
    functions = [change_from_baseline_plot(df, endpoint1), change_from_baseline_plot(df, endpoint2), severity_category_confusion_matrix(df, endpoint1, visit1='Screening', visit2=None), severity_category_confusion_matrix(df, endpoint2, visit1='Screening', visit2=None), bland_altman_plot(df, endpoint1, endpoint2, bySeverityCategory), plot_endpoint_distribution(df, endpoint1, endpoint2, bySeverityCategory=False), plot_correlation(df, endpoint1, endpoint2, bySeverityCategory=False)]
    
    if endpoint1 == gold_standard_endpoint:
        functions.append(categorized_strip_plot(df, endpoint2, gold_standard_endpoint, visit=None))
    else:
        functions.append(categorized_strip_plot(df, endpoint1, gold_standard_endpoint, visit=None))
        
    for func in functions:
        add_to_plot_results(func, plot_results=plot_results)
